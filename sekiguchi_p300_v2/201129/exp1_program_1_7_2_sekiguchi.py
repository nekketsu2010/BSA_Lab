...
class exp1:
    def __init__(self, target_word, port_name):
        ...
        self.show_time = 10 / 60
        self.duration_time = 10 / 60
        ...
        with open("P300-Speller_exp/P300-Speller_image/P300Speller_image_show_list_v1/exp_img_random_show"+str(r), 'rb') as f:
            self.light_list = pickle.load(f)[:12*n]
    ...
    def start_exp(self):
        ...
        for i in range(self.light_count):
            ...
            self.img_show(self.img[random_num], random_num, self.show_time)               # 0.16666ミリ秒間表示
            # self.time_list.append(self.timer.getTime())
            self.img_show(self.img0, marker=255, wait_sec=self.duration_time)             # 0.16666ミリ秒間非刺激時間表示
